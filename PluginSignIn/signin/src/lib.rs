...
#[no_mangle]
pub extern "system" fn Java_com_kouqurong_plugin_signin_SignInViewModel_version<'a>(
    env: JNIEnv<'a>,
    _obj: JObject<'a>,
) -> JObject<'a> {
    let api_version = unsafe { api_version() };
    let version = env
        .new_string(api_version)
...
#[no_mangle]
pub extern "system" fn Java_com_kouqurong_plugin_signin_SignInViewModel_signin<'a>(
    mut env: JNIEnv<'a>,
    _obj: JObject,
    number: jint,
    token: JString,
    ...
    let token: String = env
        ...
        .into();
    let bmp_path: String = env
        .get_string(&bmp_path)
        .expect("Couldn't get java string!")
        .into();

    let result = unsafe { api_signin(number as usize, token, bmp_path) };
