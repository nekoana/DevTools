...
class TcpClientViewModel {
    ...
    private val _connectionState =
        MutableStateFlow<IConnectionState>(IConnectionState.Disconnected)

    val address: State<String> = _address
    val isAvailableAddress: State<Boolean> = _isAvailableAddress
    val connectionState: SharedFlow<IConnectionState> =
    ...
    fun connect() = scope.launch(Dispatchers.IO) {
        runCatching {
            _connectionState.emit(IConnectionState.Connecting)
            socket = Socket(
                _address.value.split(":")[0],
                _address.value.split(":")[1].toInt()
            )
        }.onFailure {
            _connectionState.emit(IConnectionState.Error(it))
        }.onSuccess {
            _connectionState.emit(IConnectionState.Connected)
        }
    }
